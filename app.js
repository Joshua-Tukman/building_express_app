...
app.get('/api/v1/papers/:id/footnotes', (request, response) => {
  var paperId = request.params['id']
  database('papers').where('id', paperId).select()
    .then(paper => {
      if (!paper[0]) {
        return response.status(404).json({ message: "Resource not found"})
      }
      database('footnotes').where('paper_id', paperId).select()
      .then(footnotes => {
        response.status(200).json(footnotes)
      })
      .catch(error => {
        response.status(400).json({message: "There aren't any footnotes for that paper"})
      })
    }).catch(error => {
      response.status(500).json({error})
    })
